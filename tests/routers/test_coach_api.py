...
@pytest.mark.asyncio
async def test_api_get_coach_reservations(client, session, sample_coach, sample_service, sample_user):
...
async def test_api_search_available_services(client, session, sample_coach):
    ...
    response_cat = await client.get(f"/coach/services/available?category={ServiceCategory.GROUP.value}")
